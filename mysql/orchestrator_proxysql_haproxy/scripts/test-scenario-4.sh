...
echo -e "\n--------------------------------------------------------------------------"
echo "👑 TAHAP 2: Menyiapkan DB1 (mysql-master) sebagai Master Replikasi & Registrasi Orchestrator"
echo "--------------------------------------------------------------------------"
docker compose up -d orchestrator --wait
curl -s "http://localhost:3000/api/discover/mysql-master/3306" > /dev/null || true

echo -e "\n> Memeriksa status GTID & Master pada DB1..."
