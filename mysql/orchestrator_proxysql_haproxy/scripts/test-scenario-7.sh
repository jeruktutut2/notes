...
echo "=========================================================================="
echo "📌 SKENARIO 7: Enterprise Architecture (HAProxy + ProxySQL + Orchestrator + Golang REST API)"
echo "=========================================================================="
...
echo -e "\n--------------------------------------------------------------------------"
echo "⚙️ TAHAP 3: Menyalakan Orchestrator, ProxySQL, HAProxy & Golang REST API"
echo "--------------------------------------------------------------------------"
docker compose up -d orchestrator proxysql haproxy golang-app --wait
"$(dirname "$0")/04-register-orchestrator.sh"

echo -e "\n> Memeriksa Healthcheck Aplikasi REST API Golang Echo v5:"
curl -s http://localhost:8080/api/health | grep -o '"status":"UP"' && echo " ✅ Golang Echo v5 API via HAProxy Ready!"

echo -e "\n--------------------------------------------------------------------------"
echo "🚀 TAHAP 4: Uji Coba Golang REST API (Insert Biasa, Select Biasa, Transaksi Explicit)"
echo "--------------------------------------------------------------------------"

echo -e "\n1. [REST API POST /api/employees] - INSERT BIASA (HAProxy -> ProxySQL melempar ke Master)..."
curl -s -X POST http://localhost:8080/api/employees \
  -H "Content-Type: application/json" \
  -d '{"name": "Eko Enterprise (HAProxy Insert)", "position": "Principal Backend Architect"}'
echo ""

sleep 1

echo -e "\n2. [REST API GET /api/employees] - SELECT BIASA (HAProxy -> ProxySQL melempar ke Replicas)..."
curl -s http://localhost:8080/api/employees
echo ""

sleep 1

echo -e "\n3. [REST API POST /api/employees/transaction] - TRANSAKSI EXPLICIT (HAProxy -> ProxySQL Master)..."
curl -s -X POST http://localhost:8080/api/employees/transaction \
  -H "Content-Type: application/json" \
  -d '{"name": "Tx Financial Enterprise (App Tx)", "position": "Fintech Lead"}'
echo ""

sleep 1

echo -e "\n--------------------------------------------------------------------------"
echo "🔴 TAHAP 5: FAILOVER TEST (DB1 Stop -> Orchestrator & DB2 Master Baru -> Update Routing)"
echo "--------------------------------------------------------------------------"
...
curl -s -X POST http://localhost:8080/api/employees \
  -H "Content-Type: application/json" \
  -d '{"name": "Post-Failover Enterprise Data", "position": "DevOps Architect"}'
echo ""
...
echo -e "\n--------------------------------------------------------------------------"
echo "🔄 TAHAP 6: PROSES FAILBACK (Restore DB1 -> Sync GTID -> Promosi DB1 Master -> ProxySQL Routing)"
echo "--------------------------------------------------------------------------"
...
echo -e "\n5. Mengarahkan seluruh Replica (DB2, DB3) mengekor kembali ke DB1..."
docker exec -i mysql-replica1 mysql -uroot -prootpassword <<EOF
STOP REPLICA;
...
echo -e "\n--------------------------------------------------------------------------"
echo "✨ TAHAP 7: Uji Coba REST API Setelah Failback (Master Sembuh DB1)"
echo "--------------------------------------------------------------------------"
echo -e "\n1. [POST VIA REST API] Melakukan INSERT melalui Golang API ke Master Pulih (DB1)..."
curl -s -X POST http://localhost:8080/api/employees \
  -H "Content-Type: application/json" \
  -d '{"name": "Final Restored Enterprise Entry", "position": "Chief Technology Officer"}'
echo ""

sleep 1

echo -e "\n2. [GET VIA REST API] Verifikasi SELECT Akhir:"
curl -s http://localhost:8080/api/employees
echo ""

echo -e "\n🎉 SKENARIO 7 SELESAI DENGAN SUKSES! (HAProxy + ProxySQL + Orchestrator + Golang REST API)"
